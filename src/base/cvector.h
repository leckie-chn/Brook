...
//
// Define class tems CVector<Element> and CVector<Element*>
//
// CVector<Element> is designed to encapsulate C-style vectors
// (include malloc, free and memcpy-type initialization) with a
// minimum interface. Compared with std::vector, CVector ensures that
// elements are soretd successively in memory layout, and provides an
...
// (parameter of a multinomial distribution) from a symmetric
...
/*
   ...
   gsl_ran_dirichlet(gsl_rng, dirichlet_param.size(), 
